...
def test_context_menu_selects_clicked_file_and_tracks_move(tmp_path, monkeypatch):
    ...
    assert int(view.tree.set(str(source), "count")) == 3
    assert [int(view.tree.set(row, "count")) for row in matches] == [1, 2, 2]
    assert tuple(view.tree["columns"])[1:3] == ("label", "count")
    assert [view.tree.set(row, "label") for row in matches] == ["model-label", "second-label", "second-label"]
    assert [float(view.tree.set(row, "confidence")) for row in matches] == [0.9, 0.8, 0.7]
    opened = []
    ...
    assert int(view.tree.set(str(source), "count")) == 2
    assert [int(view.tree.set(row, "count")) for row in view.tree.get_children(str(source))] == [1, 1]
    view.close()
