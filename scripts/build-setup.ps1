...
& $python -c "import shutil; shutil.make_archive('dist/YOLO-media-sorter-$version-windows-$architecture-portable', 'zip', 'dist', 'YOLO-media-sorter-$version-windows-$architecture')"
$artifacts = @("dist/YOLO-media-sorter-$version-windows-$architecture-Setup.exe", "dist/YOLO-media-sorter-$version-windows-$architecture-portable.zip")
